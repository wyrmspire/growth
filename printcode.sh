...
# ---------------------------------------------------------------------------
{
    ...
    echo ""
    echo "GrowthOps OS is a Vite + TypeScript project implementing a structured business application UI powered by Genkit and Google Gemini."
    echo ""
    echo "| Area | Path | Description |"
    echo "|------|------|-------------|"
    echo "| **modules** | modules/ | Domain logic (copylab, strategy, analytics, etc) |"
    echo "| **components** | src/components/ | UI components |"
    echo "| **pages** | src/pages/ | Core pages containing UI state |"
    echo "| **docs** | *.md | SYSTEM_ARCHITECTURE, PRODUCT_DESIGN, CONTRACT |"
    echo ""
    echo "Key paths: \`src/main.ts\` (main UI entry), \`src/mock-engine.ts\` (translation layer), \`board.md\`"
    echo "Stack: Vite, TypeScript, Vanilla CSS, Genkit AI Flows"
    echo ""
